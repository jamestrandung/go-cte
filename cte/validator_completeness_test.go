...
func TestCompletenessValidator_Validate(t *testing.T) {
	...
	t.Run("doValidate return no errors", func(t *testing.T) {
		vMock.On("doValidate", mock.Anything, mock.Anything, mock.Anything).
			Return(func(planName string, cs componentStack, curPlanValue reflect.Value) error {
	...
	t.Run("doValidate return an error", func(t *testing.T) {
		vMock.On("doValidate", mock.Anything, mock.Anything, mock.Anything).
			Return(func(planName string, cs componentStack, curPlanValue reflect.Value) error {
				assert.Equal(t, v.rootPlanName, planName)
				assert.Equal(t, v.planValue, curPlanValue)
	...
	scenarios := []struct {
	...
	}{
		{
			...
			test: func(t *testing.T) {
				...
				vMock.On("verifyComponentCompleteness", mock.Anything, mock.Anything, mock.Anything, mock.Anything).
					Return(func(pm parsedMetadata, cs componentStack, componentID string, planType string) error {
		...
		},
		{
			...
			test: func(t *testing.T) {
				...
				t.Run("pointer plan value", func(t *testing.T) {
					...
					vMock.On("doValidate", mock.Anything, mock.Anything, mock.Anything).
						Return(func(pName string, cs componentStack, curPlanValue reflect.Value) error {
				...
				t.Run("non-pointer plan value", func(t *testing.T) {
					pValue := reflect.ValueOf(doValidate_ParentPlan{})
